...
![физическая схема бд](./img/db_fiz1.png)
...
| **Инфраструктура**    |                                  |                                                                                  |
| Docker                | Развёртывание приложения         | Удобное и современное решение для развёртывания приложения.                      |
...
- Как это работает:
  - Сигнал от Kubernetes: При обновлении сервиса K8s отправляет поду сигнал SIGTERM.
  - Остановка трафика: Балансировщик (L7/Ingress) перестает направлять новые запросы на этот экземпляр.
  - Завершение gRPC/HTTP запросов: Сервис на Go продолжает обрабатывать уже принятые запросы в течение заданного периода (grace period, обычно 30 секунд).
  - Kafka Consumer: Сервис перестает забирать новые сообщения из Kafka, дорабатывает текущие и вызывает Close(), что провоцирует корректный Rebalancing (передачу задач другим воркерам) и фиксацию (commit) пройденных offset-ов.
  - Закрытие соединений: Последним этапом закрываются соединения с PostgreSQL и Redis.

### 2. Graceful Degradation

При выходе из строя части системы банк должен продолжать выполнять критические функции (платежи и просмотр баланса).

- Как это работает:
  - Приоритизация сервисов: Если Credit Service или Notification Service (Kafka) недоступны, API Gateway не возвращает 500 ошибку на весь экран. Вместо этого при клике на сервис кредиты, появлется надпись: "Сервис в данное время не доступен, но вы всегда можете позвонить в нашу поддержку по номеру: +7 ()...", и если человек позвонит то сможет оставить заявку на оформления кредита, а не сидеть и ждать пока сервис починиться.
  - Использование кэша: Если основная база PostgreSQL AccountDB испытывает задержки, сервис балансов может временно отдавать данные из Redis, помечая их как «актуальные на 5 минуты назад».
  - Отложенная обработка: При отказе Kafka для уведомлений, транзакции всё равно проходят. Уведомления копятся в локальном буфере сервиса или логах и отправляются позже, когда шина данных восстановится.

### 3. Circuit Breaker

Этот паттерн предотвращает «эффект домино», когда один медленный сервис (например, внешнее БКИ) парализует всю цепочку вызовов.

- Как это работает:
  - Мониторинг ошибок: Библиотека (например, gobreaker) отслеживает количество ошибок или таймаутов при вызовах между микросервисами.
  - Три состояния:
    - Closed: Система работает штатно, запросы проходят.
    - Open: Если % ошибок превышает порог (например, 20% за 10 секунд), предохранитель «размыкается». Все последующие запросы к сервису мгновенно возвращают ошибку, не нагружая сеть и CPU.
    - Half-Open: Через некоторое время (30-60 сек) пропускается один пробный запрос. Если он успешен, система возвращается в нормальный режим.
  - Пример: Если CreditWorker долго ждет ответа от «Бюро Кредитных Историй», Circuit Breaker разорвет связь, чтобы воркеры не копились в очереди и не забивали память.

### 4. Automatic Failover Policy

Обеспечивает непрерывность работы при аппаратных сбоях в одном из дата-центров.

- Как это работает:
  - Уровень БД (PostgreSQL): Используется Patroni + etcd. В каждом ДЦ есть реплики. Если Мастер в DC1 падает, Patroni в течение 10–30 секунд выбирает новым лидером актуальную реплику. PgBouncer мгновенно перенаправляет запросы на новый IP.
  - Уровень очередей (Kafka): При падении брокера в одном ДЦ, управление партициями (partitions) переходит к контроллеру в другом ДЦ. Благодаря Replication Factor: 3, данные остаются доступными.
  - Уровень кэша (Redis): Redis Sentinel мониторит узлы. Если мастер недоступен, Sentinel назначает одну из реплик новым мастером, обновляя конфигурацию в приложении.
  - Уровень ДЦ (Anycast): Если весь DC1 уходит в оффлайн, BGP Anycast перестает анонсировать его IP-префиксы, и глобальный трафик перераспределяется между DC2 и DC3.

# Расчет ресурсов

Все сервисы развёртываются в 3 дата-центрах (DC1–DC3). Stateless-сервисы (API-шлюзы, микросервисы на Go) работают в Kubernetes.
Stateful-компоненты (PostgreSQL, Cassandra, Redis) — на выделенных физических серверах (Bare Metal) для обеспечения предсказуемого I/O и соответствия требованиям безопасности финтеха.
Расчёты выполнены для DC1 (Москва) — крупнейшего ДЦ. Для обеспечения высокой доступности в случае ЧС, мощность каждого ДЦ закладывается с запасом +50% от номинальной пиковой нагрузки.
...
### Расчет жёсткого диска

| Тип данных/база                           | Объём данных | Индексы + WAL + Overhead |  Итого  | Тип накопителя |
| :---------------------------------------- | :----------- | :----------------------: | :-----: | :------------: |
| **PostgreSQL** (Users, Accounts, Credits) | 100гб        |           x2.0           | ~600 ГБ |      NVMe      |
| **PostgreSQL (Transactions)**             | 5тб          |           x1.5           | 22.5 ТБ |      NVMe      |
| **Cassandra (Notifications)**             | 80тб         |           x1.2           | 288 ТБ  |    NVMe/SSD    |
| **Kafka (Retention 7 days)**              | 1тб          |           x1.2           | 3.6 ТБ  |      SSD       |
| **Redis (Sessions)**                      | 40гб         |           x1.1           |  88 ГБ  |   RAM + NVMe   |
| **ELK / Prometheus (Logs)**               | 20тб         |           x1.3           |  52 ТБ  |    SATA/SSD    |
| **Итого**                                 |              |                          | 367 ТБ  |                |

### Аренда

| Тип сервера        | Конфигурация (Пример: Selectel)          | Кол-во (всего) | Аренда (мес/ед) |  Итого в месяц  | Обоснования                                                                                                                                                    |
| :----------------- | :--------------------------------------- | :------------: | :-------------: | :-------------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **kubenode** (App) | 2×EPYC 7443P (24c), 128GB RAM            |       12       |    40 000 ₽     |    480 000р     | [Цена](https://selectel.ru/services/dedicated/config/?uuid=337c2fde-31ab-40e8-847a-e4784250e483)                                                               |
| **lvs-l4**         | Intel Xeon E-2236, 25GbE NIC             |       6        |     9 000 ₽     |    54 000 ₽     |
| **nginx-l7**       | Xeon Silver 4310, 25GbE NIC              |       6        |    20 000 ₽     |    120 000 ₽    |
| **db-postgres**    | 2 x AMD EPYC 7513, 256GB, 2xNVMe 1.92 TB |       15       |    62 640 ₽     |    930 000 ₽    | [Цена](https://timeweb.cloud/services/dedicated-server?type=preset&id=5851&preset_id=5851&aid=5851&term=12&location=msk&cp=0&network=1008&ip=0&storage=0&os=0) |
| **db-cassandra**   | AMD EPYC 7343, 128GB, 4xNVMe 4TB         |       24       |    33 000 ₽     |     792 000     | [Цена](https://timeweb.cloud/services/dedicated-server?type=preset&id=5851&preset_id=5851&aid=5851&term=12&location=msk&cp=0&network=1008&ip=0&storage=0&os=0) |
| **db-redis**       | Xeon 4210R, 384GB RAM                    |       9        |    22 000 ₽     |    198 000 ₽    | [Цена](https://ru.fornex.com/dedicated-servers/ds-promo-4210r-syh-nvme/)                                                                                       |
| **kafka-node**     | Intel Xeon Silver 4116, 128GB, 4xSSD 2TB |       9        |    41 000 ₽     |    369 000 ₽    | [Цена](https://ru.fornex.com/dedicated-servers/ds-highway-4116-suf/)                                                                                           |
| **Итого**          |                                          |     **81**     |                 | **2 943 000 ₽** |

### Расчёт покупки

| Тип сервера              | Конфигурация (Пример: Selectel)          | Кол-во (всего) | Цена за 1 шт. |    Итого в месяц    | Обоснования                                                                                                                                                      |
| :----------------------- | :--------------------------------------- | :------------: | :-----------: | :-----------------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **kubenode** (App)       | 2×EPYC 7443P, 128GB RAM                  |       12       |   130 000 р   |     1 560 000 ₽     | [Цена](https://serverflow.ru/catalog/komplektuyushchie/protsessory/amd-epyc-7443p/)                                                                              |
| **lvs-l4**               | Xeon E-2334, 25GbE NIC                   |       6        |    95 000     |      570 000 ₽      | [Цена](https://safedec.ru/540-bcnj) [Цена](https://e-burg.nix.ru/autocatalog/intel/CPU-Intel-Xeon-E-2334-34-GHz-4core-2-plus-8Mb-65W-8-GT-s-LGA1200_571836.html) |
| **nginx-l7**             | Xeon Silver 4310, 25GbE NIC              |       6        |    400 000    |     2 400 000 ₽     | [Цена](https://servermall.ru/catalog/servers/xfusion-2288h-v6-24sff/)                                                                                            |
| **db-postgres**          | 2 x AMD EPYC 7513, 256GB, 2xNVMe 2TB     |       15       |  5 600 000 ₽  |    84 000 000 ₽     | [Цена](https://serverflow.ru/catalog/ai-servers/supermicro-4124gs-tnr-6-a100-2-epyc-7513/)                                                                       |
| **db-cassandra**         | AMD EPYC 7343, 128GB, 4xNVMe 4TB         |       24       |  1 000 000 ₽  |    24 000 000 ₽     | [Цена](https://servermall.ru/catalog/servers/hpe-proliant-dl385-gen10-plus-v2-8sff/)                                                                             |
| **db-redis**             | Xeon 4210R (10c), 256GB RAM              |       9        |   380 000 ₽   |     3 420 000 ₽     | [Цена](https://servermall.ru/catalog/servers/xfusion-2288h-v6-24sff/)                                                                                            |
| **kafka-node**           | Intel Xeon Silver 4116, 128GB, 4xSSD 2TB |       9        |   480 000 ₽   |     4 320 000 ₽     | [Цена](https://servermall.ru/catalog/servers/dell-poweredge-r760-12lff/)                                                                                         |
| **Сетевое оборудование** | Switches 25GbE (6 шт) + Routers (3 шт)   |       -        |       -       |     5 000 000 ₽     |
| **Итого**                |                                          |     **81**     |               | **~137 500 000 ₽ ** |

### Расчёт содержания

| Статья расхода     | Расчёт                                   |  Итого в месяц  | Обоснования                                  |
| :----------------- | :--------------------------------------- | :-------------: | -------------------------------------------- |
| **Аренда стоек**   | 3 стойки (Москва/Екб) × 140 000 ₽        |    420 000 ₽    | [Цена](https://hosting.tel.ru/arenda-stoek/) |
| **Электроэнергия** | 40 кВт × 24ч × 30дн × 8.5 ₽/кВтч         |    260 000 ₽    |
| **Каналы связиы**  | 3 ДЦ × 10Gbps (L3 VPN + Internet)        |    150 000 ₽    |
| **Обслуживание**   | Сменный персонал (выезды, замена дисков) |    200 000 ₽    |
| **З/П инженеров**  | 2 SRE/DC-инженера (на поддержку железа)  |    500 000 ₽    | [З/п](https://career.habr.com/salaries.)     |
| **Итого**          |                                          | **1 470 000 ₽** |
...
- Ежемесячно: 2 943 000 ₽
- За 5 лет: 14 715 000 ₽

**Вариант B: Покупка**

- Разово: 137 500 000 ₽
- За 3 лет содержания: 88 200 000 ₽.
- Итого: 225 700 000 ₽.
...
Для обеспечения отказоустойчивости внутри каждого ДЦ, сервисы распределены минимум по 3 репликам.

| Сервис                  | Реплики | CPU Request | RAM Request | Назначение           |
| :---------------------- | :-----: | :---------: | :---------: | :------------------- |
| **auth-service**        |    4    |      1      |    2 ГБ     | Авторизация и сессии |
| **account-service**     |    6    |      1      |    2 ГБ     | Балансы и счета      |
| **transaction-service** |    5    |      1      |    2 ГБ     | Переводы и платежи   |
| **notif-service**       |    3    |     0.5     |   1.5 ГБ    | Push-уведомления     |

## Обоснование масштабирования

1.  **PostgreSQL (High Availability):** Использование 15 серверов позволяет держать в каждом из 3-х ДЦ кластер из Master + 2 Sync Slaves. Это гарантирует **RPO=0** (ни одна транзакция не будет потеряна при падении мастера).
2.  **Cassandra (Storage-heavy):** 24 ноды необходимы для хранения 80 ТБ уведомлений. С учетом фактора репликации (RF=3) и заполняемости дисков не более 70%, суммарная чистая емкость кластера составляет ~270 ТБ.
3.  **L4/L7 Балансировка:** Несмотря на низкий средний трафик, используются серверы с 25GbE интерфейсами для эффективной обработки всплесков в "дни выплат" и отражения DDoS-атак на сетевом уровне.
