...
- Как это работает:
  - Приоритизация сервисов: Если Credit Service или Notification Service (Kafka) недоступны, API Gateway не возвращает 500 ошибку на весь экран. Вместо этого в мобильном приложении просто скрывается блок «Кредиты» или выдается сообщение «Сервис временно недоступен».
  - Использование кэша: Если основная база PostgreSQL AccountDB испытывает задержки, сервис балансов может временно отдавать данные из Redis (Read-through cache), помечая их как «актуальные на 2 минуты назад».
...
- Как это работает:
  - Мониторинг ошибок: Библиотека (например, gobreaker или возможности Service Mesh типа Istio) отслеживает количество ошибок или таймаутов при вызовах между микросервисами.
  - Три состояния:
    - Closed (Закрыт): Система работает штатно, запросы проходят.
    - Open (Открыт): Если % ошибок превышает порог (например, 20% за 10 секунд), предохранитель «размыкается». Все последующие запросы к сервису мгновенно возвращают ошибку, не нагружая сеть и CPU.
    - Half-Open (Полуоткрыт): Через некоторое время (30-60 сек) пропускается один пробный запрос. Если он успешен, система возвращается в нормальный режим.
  - Пример: Если CreditWorker долго ждет ответа от «Бюро Кредитных Историй» (БКИ), Circuit Breaker разорвет связь, чтобы воркеры не копились в очереди и не забивали память.
...
- Как это работает:
  - Уровень БД (PostgreSQL): Используется Patroni + etcd. В каждом ДЦ есть реплики. Если Мастер в DC1 падает, Patroni в течение 10–30 секунд выбирает новым лидером самую актуальную реплику (обычно в DC2). PgBouncer мгновенно перенаправляет запросы на новый IP.
  ...
  - Уровень кэша (Redis): Redis Sentinel мониторит узлы. Если мастер недоступен, Sentinel проводит голосование и назначает одну из реплик новым мастером, обновляя конфигурацию в приложении через сервис-дискавери.
  - Уровень ДЦ (Anycast): Если весь DC1 уходит в оффлайн, BGP Anycast перестает анонсировать его IP-префиксы, и глобальный трафик перераспределяется между DC2 и DC3 на уровне магистральных провайдеров.
